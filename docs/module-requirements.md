# Ashfall NeoForge Edition Module Requirements

The pack manifest declares module requirements instead of hard-coding every module file by hand. The current `release-assets/v0.1.0-ashfall-neoforge-edition/ashfall-neoforge-edition-alpha-0.1.0.pack.json` declares 46 requirements for the `neoforge` module artifact family.

```json
{
  "moduleArtifactFamily": "neoforge",
  "moduleRequirements": [
    {
      "id": "echoashfallprotocol",
      "version": "1.0.0",
      "required": true
    }
  ]
}
```

The launcher resolves the default artifact name as `<module>-<version>-neoforge.jar`. Individual requirements can override `assetName`, `path`, `sha256`, `size`, `side`, or `artifactFamily`.

## Current Release Status

The checked-in `release-assets/v0.1.1-ashfall-neoforge-edition-beta.8/` files mirror the current GitHub prerelease assets and include `moduleRequirements`. Beta.8 pins `modules-ashfall-terminal-hotfix-20260621`, including the ScreenCore input fallback, Terminal tab-switch reuse, Command Deck route snapshot avoidance, and Ashfall Protocol NeoForge runtime fixes. The current Ashfall NeoForge lane smoke has real gameplay proof for client start, world load, HUD, Index, Terminal, HoloMap, Lens, creative inventory search/selection/use, and save/reload. Computer Use visual capture remains blocked by `SetIsBorderRequired failed: No such interface supported (0x80004002)`, so the accepted proof set uses launcher-imported screenshots and logs until the capture backend is fixed.

<!-- runtime-parity-baseline:start -->
## Runtime Parity Baseline

Generated by `node scripts/apply-runtime-parity-pack-baseline.mjs` from the ECHO module runtime parity audit.

- Source audit: `ECHO-Modules/reports/runtime-parity/echo-module-runtime-parity-audit.json`
- Module artifact family: `neoforge`
- Latest added baseline modules for `ECHO-Ashfall-NeoForge-Edition`: `echohudcore`, `echoindex`, `echoholomap`, `echolens`, `echoterminal`, `echoscreencore`, `echothemecore`, `echoworldcore`

Required modules after the parity sync:

- `echoadaptercore` (1.0.0)
- `echoashfallprotocol` (1.0.0)
- `echoblockworks` (1.0.0)
- `echocore` (1.0.0)
- `echoholomap` (1.0.0)
- `echohudcore` (1.0.0)
- `echoindex` (1.0.1)
- `echolens` (1.0.0)
- `echonetcore` (1.0.0)
- `echoplatformcore` (1.0.0)
- `echoscreencore` (1.0.0)
- `echoterminal` (1.0.0)
- `echothemecore` (1.3.0)
- `echoworldcore` (1.0.0)
<!-- runtime-parity-baseline:end -->
