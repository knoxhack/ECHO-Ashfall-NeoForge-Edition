# Ashfall NeoForge Edition Troubleshooting

## Module Does Not Update

Check that the pack manifest has a matching `moduleRequirements` entry, that `knoxhack/ECHO-Modules` has the requested version, and that the artifact family is `neoforge`.
